...
class Script {
    config = {
        arrayOfAccessTokens: ["b638253eab63e45fe4f42d61777f980c5bb417086afe20fbdeb9f07ab28362c90784dadef4a17d8c55618"], // array of tokens    
        ...
        wallOwnerIdentifier: 197043811, // owner identifier (user or community)
        wallPostIdentifier: 3, // post identifier
        removeDelay: 300, // remove delay (in secounds)
		groupIdentifier: 195587153
    }
        ...
        try {
            ...
            for (let accessToken of this.config.arrayOfAccessTokens) {
                try {
                    ...
                    Script.consoleLog(`${currentUserInfo.first_name} ${currentUserInfo.last_name} запущен`);
                    Script.consoleLog(`Репостнул запись: vk.com/wall${this.config.wallOwnerIdentifier}_${this.config.wallPostIdentifier}`);

                    let responseRepostMethod = await this.requestAPI("wall.repost", {
                        object: `cdpiar?w=wall-${this.config.wallOwnerIdentifier}_${this.config.wallPostIdentifier}`,
                        group_id: this.config.groupIdentifier,
                        message: this.config.messageOfPost,
                        access_token: accessToken
                    });

                    Script.consoleLog(`Запись (vk.com/wall${this.config.wallOwnerIdentifier}_${this.config.wallPostIdentifier}) опубликована!`);
                    ...
                    Script.consoleLog(`Запись (vk.com/wall${this.config.groupIdentifier ? -this.config.groupIdentifier : currentUserInfo.id}_${responseRepostMethod.post_id}) будет удалена через ${this.config.removeDelay} сек.`);
                    ...
                    setTimeout(
                        () => (
                            ...
                            Script.consoleLog(`Запись (vk.com/wall${this.config.groupIdentifier ? -this.config.groupIdentifier : currentUserInfo.id}_${responseRepostMethod.post_id}) удалена!`)
                        ),
